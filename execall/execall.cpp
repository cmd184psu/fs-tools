...
void lookfordups(cString wildcard, cString execall, bool usedirs) {
	...
	for(int i=0; i<thelist.Length(); i++) {
		if(thelist[i]!="") {
				...
				cout<<execall<<" "<<BashFriendly(thelist.GetName(i))<<endl;
...
int main(int argc, char* argv[]) {
	...
	bool useexec=arguments.IsSet("exec");
	
	if(!useexec) {
		cerr<<"Missing execution parameter"<<endl;
		exit(1);
	}
	
	cString execall=arguments.GetArg("exec",1);
	...
	if(!wildcard.Contains(';')) {
		 if(arguments.IsSet("d") || arguments.IsSet("b")) lookfordups(wildcard, execall, true);
		 if(!arguments.IsSet("d") || arguments.IsSet("b")) lookfordups(wildcard, execall, false);
		 
	} else {
		wildcardlist.FromString(wildcard,';');
		for(int i=0; i<wildcardlist.Length(); i++) lookfordups(wildcardlist[i], execall, usedirs);
	}
