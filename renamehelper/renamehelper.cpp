			...
			/*for(int j=i+1; j<thelist.Length(); j++) {
				...
				*/
				...
				newfile=curfile.Replace("\'","_").
					...
					Replace("[","_").
					Replace("]","_").ToLower();
				if(isone(newfile)) newfile=fixit(newfile);	
				while(newfile.Contains("__")) newfile=newfile.Replace("__","_");	
				stat(thelist.GetName(i),&statbuf);	
				tad=(&statbuf.st_mtime);	
				
cout<<"(cd \""<<dir<<"\" && mv -vi \""<<curfile.Replace("$","\\$")<<"\" \'"<<newfile.ChopRt('.')<<"_"<<
tad.GetDay().ofthemonth<<
tad.GetMonthString().ChopRt(tad.GetMonthString().Length()-3)<<
tad.GetYear()+1900<<"."<<newfile.ChopLf('.')<<"\')"<<endl;
...
int main(int argc, char* argv[]) {
	...
	if(arguments.IsSet("x")) {
		cerr<<"do something else"<<endl;
		
		if(wildcard.Contains('*')) {
			cerr<<"can't do wildcards for this"<<endl;
			exit(1);
		}
